...
int dispcmd(interpreteur inter, memory mem)
{
	...
	while (token != NULL) {
 	    ...
 	    char* tok_mem1 = strtok(token, ":");
	    char* tok_mem2;
	    if (is_adress(tok_mem1)) {
	        if ((tok_mem2 = strtok(NULL, ":")) == NULL) {
		    ...
		    size_t adress2 = atoi(tok_mem2);
	            ...
	            int j = 0;
		    byte value;
 	            if ((value = read_memory_value(adress1, mem)) != NULL)
			     printf("%zx\t%x  ", adress1, value);
		    else {
		        WARNING_MSG("adress %zx not found %s\n",adress1, "dispcmd");
		        return 1;
		    }
		    for(j = 1; adress1 + j <= adress2; j++) {
		        if ((value = read_memory_value(adress1 + j, mem)) != NULL)
			     printf("%x  ", value);
	 	        else {
		            WARNING_MSG("adress %zx not found %s\n",adress1 + j, "dispcmd");
		            return 1;
